...
def main():
    ...
    for sid in STATION_IDS:
        try:
            ...
            output = {"obs": obs, "ai_expert": {"briefing": "AI 數據部署成功", "safety_score": 90, "activities": ["釣魚"]}}
            with open(f"deploy/edge_{sid}.json", "w", encoding="utf-8") as f:
                json.dump(output, f, ensure_ascii=False)
            print(f"✅ edge_{sid}.json 已生成")
        except Exception as e: print(f"❌ {sid} 失敗: {e}")
